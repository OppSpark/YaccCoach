...
const DiseaseManager = () => {
  ...
  const handleAdd = async () => {
    ...
    try {
      await axios.post('/disease', { user_id: userId, disease_name: newDisease });
      setNewDisease('');
      setRefresh(!refresh);
    } catch (err) {
      setError('추가에 실패했어요.');
            ...
            {editDiseaseId === disease.disease_id ? (
              <>
                <input
                  ...
                  className="input"
                />
                <button className="edit-btn" onClick={() => handleUpdate(disease.disease_id)}>저장</button>
                <button className="cancel-btn" onClick={() => setEditDiseaseId(null)}>취소</button>
              </>
            ) : (
              <>
                ...
                <button onClick={() => handleDelete(disease.disease_id)}>🗑️</button>
              </>
            )}
          </div>
        ))}
